                ...
                <div class="row">
                    ...
                    <div class="ulproductdetail" style="padding:24px">
                    	...
                    	<ul class="nav nav-tabs">
                        	<li class="active">
                                <a href="#info_ssl" data-toggle="tab">Info SSL</a>
                            </li>
                            <li{if $reissue_ssl} class="active"{/if}>
                                <a href="#reissue_ssl" data-toggle="tab">Reissue SSL</a>
                            </li>
                            <li{if $renew_ssl} class="active"{/if}>
                                <a href="#renew_ssl" data-toggle="tab">Renew SSL</a>
                            </li>
                    	</ul>
                    	<div class="tab-content product-details-tab-container">
                        	<div class="tab-pane fade in active text-center" id="info_ssl">
                        	    <h5 align="left" style="margin-bottom:15px">SSL Certificate Status : <strong>Active</strong></h5>
                        	    <h5 align="left" style="margin-bottom:15px">Expire Date : <strong>{$exp_date}</strong></h5>
                        	    <h5 align="left" style="margin-bottom:15px">Certificate : <a href="{$certificate}" target="_blank">Download Certificate</a></h5>
                        	    <h5 align="left" style="margin-bottom:15px">CA Bundle : <a href="{$ca_bundle}" target="_blank">Download CA Bundle</a></h5>
                        	</div>
                        	<div class ="tab-pane fade in {if $reissue_ssl} active {/if} text-center" id="reissue_ssl">
                                <div style="display:none; grid-template-columns: 200px auto; text-align:left; margin-bottom:15px;width:50%">
                                    <p>Contact:</p>
                                ...
                                <form method="post" action="clientarea.php?action=productdetails&id={$id}">
                                    ...
                                    <input type="hidden" value="reissue_ssl" name="reissue_ssl"/>
                                    
                                    <div style="display:grid; grid-template-columns: 200px auto; text-align:left; margin-bottom:15px;width:50%">
                                        <p>CSR Reissue:</p>
                                        <div>
                                    ...
                                    <div style="display:grid; grid-template-columns: 200px auto; text-align:left;width:50%; margin-bottom:15px">
                                        <p>Validation Method:</p>
                                        <select id="validationMethod" onchange="handlerChangeValidation(event, 'approval_reissue')" name="validation_method" style="padding:8px; border-radius:8px; border:1px solid #ced4da">
                                            <option value="email" selected>Email</option>
                                            <option value="dns">DNS</option>
                                        </select>
                                    </div>
                                    
                                    <div id="approval_reissue" style="display:grid; grid-template-columns: 200px auto; text-align:left;width:50%; margin-bottom:15px">
                                        <p>Approval Email:</p>
                                    ...
                                    {if $disable === true}
                                        <div style="width:50%; text-align:right; margin-top:15px; padding-bottom:25px">
                        	...
                        	<div class ="tab-pane fade in {if $renew_ssl} active {/if} text-center" id="renew_ssl">
                        	    <form method="post" action="clientarea.php?action=productdetails&id={$id}">
                                    <input type="hidden" value="{$myContact}" name="contact" id="contactId"/>
                                    <input type="hidden" value="1" name="disable"/>
                                    <input type="hidden" value="renew_ssl" name="renew_ssl"/>
                                    ...
                                    <div style="display:grid; grid-template-columns: 200px auto; text-align:left;width:50%; margin-bottom:15px">
                                        <p>Validation Method:</p>
                                        <select id="validationMethod" onchange="handlerChangeValidation(event, 'approval_renew')" name="validation_method" style="padding:8px; border-radius:8px; border:1px solid #ced4da">
                                            <option value="email" selected>Email</option>
                                            <option value="dns">DNS</option>
                                        </select>
                                    </div>
                                    
                                    <div id="approval_renew" style="display:grid; grid-template-columns: 200px auto; text-align:left;width:50%; margin-bottom:15px">
                                        <p>Approval Email:</p>
                                        <select id="approvalEmail" name="approval_email" style="padding:8px; border-radius:8px; border:1px solid #ced4da">
                                            {foreach $dataApproval as $approval}
                                                <option value="{$approval}" {if $loop.first}selected{/if}>{$approval}</option>
                    	...
                    	{else if $status_irsfa === 'REJ'}
                    	<h5 align="left" style="margin-bottom:15px">Please Contact Administrator</a></h5>
                    	{else if $status_irsfa === 'REQ'}
                    	<h5 align="left" style="margin-bottom:15px">Please Contact Administrator</a></h5>
                	    {else if $status_irsfa === 'FAI'}
                        <h5 align="left" style="margin-bottom:15px">Please Contact Administrator</a></h5>
                    	{else if $status_irsfa === 'DEL'}
                    	<h5 align="left" style="margin-bottom:15px">Please Contact Administrator</a></h5>
                    	{else if $status_irsfa === 'EXP'}
                    	<h5 align="left" style="margin-bottom:15px">Please Contact Administrator</a></h5>
                    	{else}
                    	<h5 align="left" style="margin-bottom:15px">Langkah 1 : <strong>Selesai</strong></h5>
                	    <h5 align="left" style="margin-bottom:15px">Langkah 2 : Tata cara verifikasi ssl Anda akan dikirimkan melalui email <span style="font-weight:bold;">{$email_client}</span></h5>
                    	{/if}
                       </div>
                    </div>
                </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
              ...
              <form method="post" action="clientarea.php?action=productdetails&id={$id}">
                            ...
                            <div style="width:50%;padding-right:5px">
                                ...
                                <select id="listCountry" name="country" style="padding:8px; border-radius:8px; border:1px solid #ced4da; width:100%; margin-bottom:10px">
                                    {foreach $dataCountry as $country}
                                        <option value="{$country.iso}" {if $loop.first}selected{/if}>{$country.name}</option>
                                    {/foreach}
                                </select>
                            </div>
                            <div style="width:50%;padding-left:5px">
                  ...
                  <div class="modal-footer">
                    <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
                    <button id ="btnactionmoveservice" class="btn btn-default submitmoveservice">Submit</button>
                  </div>
              </form>
        </div>
     </div>
...
<script>
    const id = '{$id}';
    function handlerChangeValidation(e, id){
        const el_approve =  document.getElementById(id);
